function retirarIdFila(id) {
	try {
		...
		if (numRowsToUpdate > 0) {
		    ...
		    for (let i = 1; i < data.length; i++) {
		        let sameDate = false;
		        if (data[i][3] instanceof Date && dia instanceof Date) {
		            sameDate = data[i][3].getTime() === dia.getTime();
		        } else {
		            sameDate = String(data[i][3]) === String(dia);
		        }
		        if (sameDate) {
...
function procesarFila(fecha) {
	...
	for (let i = 1; i < displayData.length; i++) {
	    if (sonFechasIguales(displayData[i][3], fecha)) {
	        filaEspera.push({
	            index: i + 1,
